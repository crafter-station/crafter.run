...
2. Check authentication with `crafter whoami`. If needed, run `crafter login`. If it returns `member: null`, help the user create their Crafter profile: gather `handle` and `displayName` (plus any optional fields they provide), check availability with `crafter handle <handle>`, show the exact JSON, and only after explicit approval run `crafter onboard --file <json-file> --confirm`. Bare `crafter onboard` opens the browser form instead. Full onboarding instructions live at [crafter.run/join/agent.md](https://crafter.run/join/agent.md).
