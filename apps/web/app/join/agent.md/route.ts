...
- If their browser or terminal shows \`invalid_client\` or "The requested OAuth 2.0 Client does not exist", the CLI is pointed at an OAuth client that Crafter's identity provider does not have. Ask the user to run \`env | grep CRAFTER_\` and unset everything it prints, including any \`CRAFTER_CLI_OAUTH_*\` override and any legacy \`CRAFTER_OAUTH_*\` variable left over from running the Crafter API locally, then reinstall with \`npm install --global @crafter/cli@latest\` and retry \`crafter login\`. Never set those variables yourself to work around the error: pointing the CLI at a different issuer cannot fix a missing client and will send the user's sign-in to the wrong place.
